...
def make_height_map_vertices_and_normals(height_map, spacing=1):
    ...
    vertices = vertices.at[:,:,0].set(
        spacing_x * (jnp.arange(bw)[None,:] - bw/2. + 0.5))
    vertices = vertices.at[:,:,1].set(
        spacing_y * (jnp.arange(bh)[:,None] - bh/2. + 0.5))
    ...
    vertices = vertices.at[ 0,:, 1].add(0.5 * spacing_y)
    vertices = vertices.at[-1,:, 1].add(-0.5 * spacing_y)
    vertices = vertices.at[:, 0, 0].add(0.5 * spacing_x)
    vertices = vertices.at[:,-1, 0].add(-0.5 * spacing_x)
