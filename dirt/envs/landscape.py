...
class LandscapeParams:
    ...
    air_initial_temperature : float = 25.
...
def landscape_naive(
    ...
    for step_size in params.step_sizes:
        def step(
            ...
            air_smell = state.air_smell
            air_temperature = state.air_temperature
            air_light = jnp.full(params.world_size, params.light_initial_strength, dtype=dtype)
            day = state.day
            ...
            # Day_status
            day += 1
            day_status = get_day_status(day_length, day_length / 2, day)
