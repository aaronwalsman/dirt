...
from dirt.envs.tera_arium.landscape import (
    LandscapeParams,
    make_landscape,
)
...
def make_tera_arium(
    ...
    class TeraArium:
        ...
        @static_data
        class State:
            landscape_state : landscape.State
            bug_state : bugs.State
            bug_traits : bugs.Traits
        ...
        def init_state(
            ...
            homicide_locations = jnp.zeros(
                (params.max_players, 2), dtype=jnp.int32)
            
            state = TeraArium.State(
                landscape_state,
                bug_state,
                bug_traits,
        ...
        def transition(
            ...
            landscape_state = state.landscape_state
        ...
        def observe(
            ...
            if params.vision_includes_rgb:
                rgb = landscape.render_rgb(
                    state.landscape_state,
                    params.world_size,
                    spot_x=state.bug_state.x,
            ...
            if params.vision_includes_relative_altitude:
                altitude = landscape.get_altitude(state.landscape_state)
            ...
            if params.include_wind:
                wind = state.landscape_state.wind/state.landscape_state.max_wind
                wind = jnp.repeat(
            ...
            if params.include_temperature:
                temperature = read_grid_locations(
                    state.landscape_state.temperature,
                    state.bug_state.x,
            ...
            # external resources
            if params.include_water:
                external_water = landscape.get_water(
                    state.landscape_state, state.bug_state.x)
            else:
                external_water = None
            external_energy = read_grid_locations(
                state.landscape_state.energy,
                state.bug_state.x,
                params.landscape.resource_downsample,
            )
            external_biomass = read_grid_locations(
                state.landscape_state.biomass,
                state.bug_state.x,
                params.landscape.resource_downsample,
            )
        ...
        def active_players(state):
            return bugs.active_players(state.bug_state)
        
        def family_info(state):
            return bugs.family_info(state.bug_state)
        
        def correct(state, steps):
            if params.auto_correct_biomass:
                #state_biomass = (
                #    jnp.sum(state.landscape.biomass, dtype=jnp.float32) +
                #    jnp.sum(state.bug_state.biomass, dtype=jnp.float32)
                #)
                state_landscape_biomass = jnp.sum(
                    state.landscape_state.biomass, dtype=jnp.float32)
                state_bugs_biomass = jnp.sum(
                    state.bug_state.biomass, dtype=jnp.float32)
                ...
                #target_landscape_biomass = (
                #    state_biomass - next_state_bugs_biomass)
                #loss_ratio = (
                #    next_state_landscape_biomass / target_landscape_biomass)
                #step_biomass_loss_ratio = (
                #    loss_ratio ** (20./params.steps_per_epoch))
        ...
        class VisualizerReport:
            ...
            if params.report_bug_traits:
                traits : bugs.Traits = bugs.Traits.default(())
        
        def default_visualizer_report():
            return TeraArium.VisualizerReport()
        
        def make_visualizer_report(state, actions):
            report = TeraArium.VisualizerReport(
                players=TeraArium.active_players(state),
                player_x=state.bug_state.x,
                player_r=state.bug_state.r,
                player_color=state.bug_traits.color,
            )
            if params.include_rock:
                report = report.replace(rock=state.landscape_state.rock)
            if params.include_water:
                report = report.replace(water=state.landscape_state.water)
            if params.include_light:
                report = report.replace(light=state.landscape_state.light)
            if params.include_temperature:
                report = report.replace(
                    temperature=state.landscape_state.temperature)
            if params.include_rain:
                report = report.replace(
                    moisture=state.landscape_state.moisture,
                    raining=state.landscape_state.raining,
                )
            if params.include_energy:
                report = report.replace(energy=state.landscape_state.energy)
            if params.include_biomass:
                report = report.replace(biomass=state.landscape_state.biomass)
            
            if params.report_bug_actions:
                report = report.replace(actions=actions)
            if params.report_bug_internals:
                report = report.replace(age=state.bug_state.age)
                report = report.replace(generation=state.bug_state.generation)
                report = report.replace(hp=state.bug_state.hp)
                if params.include_water:
                    report = report.replace(player_water=state.bug_state.water)
                if params.include_energy:
                    report = report.replace(
                        player_energy=state.bug_state.energy)
                if params.include_biomass:
                    report = report.replace(
                        player_biomass=state.bug_state.biomass)
            
            if params.report_bug_traits:
                report = report.replace(traits=state.bug_traits)
            
            if params.report_object_grid:
                report = report.replace(object_grid=state.bug_state.object_grid)
        ...
        def print_player_info(player_id, report):
            ...
            if params.report_bug_actions:
                (
                    action_type,
                    action_primitive,
                ) = bugs.get_action_type_and_primitive(
