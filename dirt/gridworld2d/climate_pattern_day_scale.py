...
def get_normalize_small_scale(
    array: jnp.ndarray,
    mid: float = 0.5,
    scale: float = 0.1
) -> jnp.ndarray:
    array_norm = get_normalize(array)
    return mid - scale + array_norm * scale * 2
...
def absorb_temp(
    ...
    absorption_rate = temperature_scale * light_intensity * (1 - (1 - rain_effect) * rain_status) * (1 + water_effect - water_norm) * (1 + evaporation_effect - current_evaporation_norm)
...
def release_temp(
    ...
    release_rate_norm = get_normalize(release_rate)
    return temperature * (1 - release_rate_norm)
...
def temperature_step(
    ...
    '''
    return jnp.where(time % day_length <= day_light_length, absorb_temp(light_intensity, water, temperature, rain_status, current_evaporation, water_effect, rain_effect, evaporation_effect, temperature_scale), release_temp(light_intensity, water, temperature, rain_status, current_evaporation, night_effect, water_effect, rain_effect, evaporation_effect)
)
...
def simulate_full_weather_day(
    ...
    def step_fn(carry, step_idx):
        ...
        # 4. Temperature
        new_temperature = temperature_step(day_length, current_time, water, current_temperature, rain_status, light_intensity, current_evaporation, day_light_length, night_effect, water_effect, rain_effect, evaporation_effect, temperature_scale)
...
if __name__ == '__main__':
    ...
    final_terrain, final_water, left_evaporation, final_rain_status, final_erosion, final_day_status, final_light_intensity, final_temperature = simulate_full_weather_day(
        ...
        flow_rate, erosion_ratio, erosion_endurance, light_strength, light_length, night_effect, water_effect, rain_effect, evaporation_effect, temperature_initial_value
    )
