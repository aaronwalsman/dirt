...
from mechagogue.pop.natural_selection import (
    natural_selection, NaturalSelectionParams, NaturalSelectionState)
...
@static_dataclass
class NomNomReport:
    # state : Any = NaturalSelectionState(0,0,0)
...
def run_experiment(ep, output_directory, prng_key=5432):
  key = jrng.key(prng_key)
  os.makedirs(os.path.dirname(output_directory), exist_ok=True)
  env_params = NomNomParams(
      max_energy=ep['max_energy'],
      mean_initial_food=ep['mean_initial_food'], #8**2,
      max_initial_food=ep['max_initial_food'], #32**2,
      mean_food_growth=ep['mean_food_growth'], #2**2,
      max_food_growth=ep['max_food_growth'], #16**2,
      initial_players=ep['initial_players'],
      max_players=ep['max_players'],
      world_size=ep['world_size'],
      senescence=ep['senescence'],
  )
  train_params = NaturalSelectionParams(
      max_population=ep['max_players'],
  )
  params = NomNomTrainParams(
      env_params=env_params,
      train_params=train_params,
      epochs=ep['epochs'],
      steps_per_epoch=ep['steps_per_epoch'],
...
if __name__ == '__main__':
    ...
    experiment = {
      'name' : 'senescence_variations_mult',
      'field' : 'senescence',
      'values' : jnp.linspace(0, 0.4, 5),
    ...
    exp_params = {
      'max_players' : 256,
      'max_energy' : 4.,
      'mean_initial_food' : 1000,
      'max_initial_food' : 1000,
      'mean_food_growth' : 4,
      'max_food_growth' : 1000,
      'initial_players' : 8,
      'world_size' : (32,32),
      'senescence' : 0.02,
      'epochs' : 4,
    ...
    f = experiment['field']
    vs = experiment['values']
    name = experiment['name']
    for v in vs:
      exp_params[f] = v
      print(f'Running simulation with {f}={v}.')
      for i, key in enumerate(experiment['prng_keys']):
        run_experiment(exp_params, f'./experiments/{name}/{f}={v}/run{i}/', key)
